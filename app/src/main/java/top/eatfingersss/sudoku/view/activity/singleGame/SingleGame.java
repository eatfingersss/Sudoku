package top.eatfingersss.sudoku.view.activity.singleGame;
...
import top.eatfingersss.sudoku.model.entity.*;
import top.eatfingersss.sudoku.control.Sudoku;
...
//                Snackbar.make(view, "哈哈哈哈我也不知道要说什么", Snackbar.LENGTH_LONG)
...
//        });
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case android.R.id.home:
                if(!saveData().isSuccess) return false;//保存数据
    ...
    @Override public boolean onKeyDown(int keyCode, KeyEvent event){
        if(!saveData().isSuccess)return false;
    ...
    private void matrixGet() {
        Matrix matrix = (Matrix) getIntent()
                .getSerializableExtra("matrix");
